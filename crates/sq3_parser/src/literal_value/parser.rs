...
impl<'a> LiteralParser<'a> {
    ...
    pub fn run(&mut self) -> ParserResult<NumericLiteral> {
        if self.input.contains('X') || self.input.contains('x') {
            ...
            if chars.len() > 0 {
                ...
                Err(Sq3ParserError(format!("Not a NumericLiteral")))
            }
        }
    }
    fn parse_hex(&mut self) -> ParserResult<NumericLiteral> {
        let parsed_hex = parse_sqlite_hex_to_i64(self.input)?;
        Ok(NumericLiteral::Hexadecimal(parsed_hex))
    }
    fn parse_float(&mut self) -> ParserResult<NumericLiteral> {
        let parsed_float = self.input.parse::<f32>()?;
        Ok(NumericLiteral::Float(parsed_float))
    }
    fn parse_integer(&mut self) -> ParserResult<NumericLiteral> {
...
fn parse_sqlite_hex_to_i64(hex_str: &str) -> ParserResult<i64> {
